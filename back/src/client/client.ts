...

  {
    ...
    if (args.task) {
      ...
      if (args.m) {
        ...
        let index = await fs.readFileSync(`${path}/index.ts`, {
          encoding: "utf-8",
        });
        index = index.replace(
          "@import_models",
      ...
      if (args.s) {
        ...
        let file = await fs.readFileSync(template_file, { encoding: "utf-8" });
        while (file.includes("<Service>"))
          file = file.replace("<Service>", `${Name}`);
        while (file.includes("<service>"))
          file = file.replace("<service>", name);
        ...
        if (await !fs.existsSync(`${path}/${name}`))
      ...
      if (args.c) {
        ...
        let file = await fs.readFileSync(template_file, { encoding: "utf-8" });
        while (file.includes("<Controller>"))
          file = file.replace("<Controller>", `${Name}`);
        while (file.includes("<controller>"))
          file = file.replace("<controller>", name);
      ...
      if (args.r) {
        ...
        let file = await fs.readFileSync(template_file, { encoding: "utf-8" });
        while (file.includes("<Route>"))
          file = file.replace("<Route>", `${Name}`);
        ...
        let index = await fs.readFileSync(
          `${process.cwd()}/src/app/router/index.ts`,
          { encoding: "utf-8" }
        );
        index = index.replace(
          "@import_routes",
